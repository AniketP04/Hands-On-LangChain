...
#### Why Memory Matters?
- **Context Preservation**: Maintains conversation history for coherent multi-turn conversations
- **Personalization**: Remembers user preferences and past interactions
- **State Management**: Tracks conversation state across sessions
- **Scalability**: Different memory types for different use cases (short vs long conversations)

#### Types of Memory Explored

##### 1️⃣ **ConversationBufferMemory**
Stores **all messages** in the conversation history. Simple but memory grows indefinitely.

**Best for:** Short conversations, complete context needed, debugging.
...
response = conversation.invoke({"input": "What is AI?"})
```

##### 2️⃣ **ConversationBufferWindowMemory**
Stores only the **last K messages** (sliding window approach).

**Best for:** Recent context only, memory constraints, focused conversations.
...
# Only keeps last 2 messages
```

##### 3️⃣ **ConversationSummaryMemory**
**Progressively summarizes** the conversation history using the LLM itself.

**Best for:** Long conversations, high-level context preservation, cost efficiency.
...
**Hybrid approach** - keeps recent messages intact, summarizes older ones.

**Best for:** Balanced approach, recent details + historical overview.
...
conversation = ConversationChain(llm=llm, memory=summary_buffer_memory)
```

##### 5️⃣ **ConversationTokenBufferMemory**
Stores messages based on **token limit** rather than message count.

**Best for:** Cost control, API token limits, budget-conscious applications.
...
#### 🧠 Key Learnings
- **Buffer Memory**: Simple but unbounded growth
- **Window Memory**: Fixed-size sliding window for recent context
- **Summary Memory**: LLM-powered summarization for long conversations
- **Summary Buffer**: Best of both worlds (recent + summarized)
- **Token Memory**: Cost-aware memory management

**Memory Selection Guide:**
- Short conversations → Buffer or Window Memory
- Long conversations → Summary or Summary Buffer Memory
- Cost-sensitive → Token Buffer Memory
- Complete context needed → Buffer Memory
